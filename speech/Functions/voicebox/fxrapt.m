function [fx,tt,sv]=fxrapt(s,fs,mode)
...
for iframe=1:nframe       % loop for each frame (~10 ms)
    ...
    [ipkd,vpkd]=findpeaks_VB(nccfd,'q');
...
if ~nargout || any(mode=='g')
    ...
    %%%%% changes made by hardik for calculating voiced region %%%%%%
    ax(1)=subplot(211);
    ...
    title('Speech');
    axis('tight')
    legend('voiced segments','unvoiced segments')
    ax(2)=subplot(212);
%     plot((tf+(kframe+1)/2)/fs,lagval*1000/fs,'xr',(tf+(kframe+1)/2)/fs,blag*1000/fs,'-b')
    plot((tf+(kframe+1)/2)/fs,blag*1000/fs,'-b')  % changes made by hardik on 15-05-2013
    xlabel('Time (s)');
    ylabel('Period (ms)');
%     title('Lag Candidates');  % changes made by hardik on 15-05-2013
    title('Pitch Period')
    axis('tight')
